...
import { Image, ScrollView, StatusBar, Text, View, TextInput, TouchableOpacity, Keyboard } from 'react-native';
...
class LoanDetails extends Component {
  ...
  isDobValid() {
    this.setState({ dobValid: this.state.dateOfIncorporationText !== '' ? true : false })
  }
  ...
  isProcessingFees(text) {
    ...
    if (this.state.pfAmountType.value === 'Number') {
      const pincodeRegex = /^\d+$/;
      if (text != '' && text != null && pincodeRegex.test(text) && text >= this.state.pfAmountMaster && (parseInt(text) > parseInt(this.state.pfAmount1) || parseInt(text) == parseInt(this.state.pfAmount1))) {
        valid = true;
      }
      this.setState({
        processingFees: {
  ...
  isStampDuty(text) {
    let valid = false;
    const pincodeRegex = /^\d+$/;
    if (text != '' && text != null && pincodeRegex.test(text) && Number(text) >= Number(this.state.stampDutyMaster)) {
      valid = true;
    }
    this.setState({
      stampDuty: {
  ...
  isNachCharges(text) {
    ...
    else if (text != '' && text != null && pincodeRegex.test(text) && Number(text) >= Number(this.state.nachChargesMaster)) {
      valid = true;
    }
        ...
        this.setState({
          ...
          this.props.loanVehicleDETAILS({
            ...
            callback: (response) => {
              ...
              this.setState({ dropDownItemVehicle: [...tempVehicleArray] || [] });
            },
          });
          ...
          this.props.getQDEData({
            ...
            callback: (response) => {
              this.setState({
                masterState: response?.additionalDetails?.kycaddresDetails?.state,
              ...
              if (
                ...
                if (loanData?.dealerPayouttype === 'Number' || loanData?.dealerPayouttype === 'number') {
                  this.setState({
                    dealerPayoutType: {
                      isValid: true,
                      label: "Number",
                ...
                else if (loanData?.dealerPayouttype === 'Percent' || loanData?.dealerPayouttype === 'percent') {
                  this.setState({
                    dealerPayoutType: {
                      isValid: true,
                      label: "Percent",
                ...
                this.setState(
                  {
                    selectedSourceType3: loanData?.isOpdCoverage ? 'Yes' : "No",
                    opdAmount: loanData?.opdCoverage,
                    selectedSourceType4: loanData?.isOpdAddedInLoan ? 'Yes' : "No",
                    ...
                    amountSelected: loanData.amountSelected || 0,
                    ...
                    valueRateofInterest: {
                      value: loanData.dealerCharges == undefined ? null : loanData?.rateOfInterest?.toString(),
                      isValid: true,
                    },
                    loanVehicleDetails: {
                      isValid: true,
                    ...
                    dealerPayout: {
                      value: loanData.dealerPayout == undefined ? '' : loanData.dealerPayout.toString() || null,
                      isValid: true,
                    },
                    dealerCharges: {
                      isValid: true,
                      value: loanData.dealerCharges == undefined ? '' : loanData.dealerCharges.toString() || null,
                    },
                    dealerSubvention: {
                      isValid: true,
                      value: loanData.dealerSubvention == undefined ? '' : loanData.dealerSubvention.toString() || null,
                    },
                    adminFees: {
                      isValid: true,
                      value: loanData.adminFees == undefined ? '' : loanData.adminFees.toString() || null,
                    },
                    exShowroomPrice: {
                      isValid: true,
                      value: loanData.exShowroom == undefined ? '' : loanData.exShowroom.toString() || null,
                    },
                    insurance: {
                      isValid: true,
                      value: loanData.insurance == undefined ? '' : loanData.insurance.toString() || null,
                    },
                    nachCharges: {
                      isValid: true,
                      value: loanData.nachCharges == undefined ? '' : loanData.nachCharges.toString() || null,
                    },
                    preEMI: {
                      isValid: true,
                      value: loanData.preEmi == undefined ? '' : loanData.preEmi.toString() || null,
                    },
                    processingFees: {
                      isValid: true,
                      value: loanData.processingFees !== undefined ? loanData.processingFees.toString() : null,
                    },
                    convenienceCharges: {
                      isValid: true,
                      value: loanData.convenienceCharges !== undefined ? loanData.convenienceCharges.toString() : null,
                    },
                    podCharges: {
                      value: loanData.pddCharges !== undefined ? loanData.pddCharges.toString() : null,
                      isValid: true,
                    },
                    stampDuty: {
                      value: loanData.stampDuty !== undefined ? loanData.stampDuty.toString() : null,
                      isValid: true,
                    },
                    bureauCharges: {
                      isValid: true,
                      value: loanData.bureauCharges !== undefined ? loanData.bureauCharges.toString() : null,
                    },
                    otherCharges: {
                      isValid: true,
                      value: loanData.otherCharges !== undefined ? loanData.otherCharges.toString() : null,
                    },
                    processingCharges: loanData.otherCharges !== undefined ? loanData.otherCharges.toString() : '',
                    premiumAmount: {
                      value: loanData.premiumAmount !== undefined ? loanData.premiumAmount.toString() : '',
                      isValid: true,
                    },
                    nameNominee: {
                      value: loanData.name || '',
                    ...
                    dobValid: true,
                    selectedSourceType2: loanData.isPremiumamount ? 'Yes' : 'No',
                    selectedSourceType: loanData.islifeInsurance ? 'Yes' : 'No',
                    selectedSubDealer: {
                      isValid: true,
                    ...
                    this.props.getSchemeCode({
                      ...
                      callback: (response) => {
                        ...
                        Object.keys(response.data).map((item, index) => {
                          ...
                          response.data[item].schemeCode == loanData.schemeCode ?
                            temp = response.data[item].dealerName : null
                      ...
                      this.props.loanVehicleBRAND({
                        ...
                        callback: (response) => {
                          ...
                          this.setState(
                            ...
                            () => {
                              if (this.state.loanVehicleBrand.label !== null) {
                                this.props.loanVehicleMODEL({
                                  ...
                                  callback: (response) => {
                                    ...
                                    this.setState(
                                      ...
                                      () => {
                                        if (
                                          this.state.loanVehicleModel.label !== null
                                        ) {
                                          this.props.loanVehicleSubMODEL({
                                            ...
                                            callback: (response) => {
                                              ...
                                              this.setState({
                                                ...
                                                loanVehicleSubModel: {
                                                  isValid: true,
                                                  value: loanData?.submodel || null,
                                                  label: loanData?.submodel || null,
                                                },
                                              },
                                                () => {
                                                  this.props.getSchemeCode({
                                                  ...
                                                  if (this.state.loanVehicleSubModel.label !== null) {
          ...
          {!this.state.schemeNameValid && (
            <Text style={errorLabel}>
              {this.state.schemeName != ''
                ? "Invalid scheme name" : 'Mobile Number is mandatory'}
            </Text>
          )}
        </View>
    ...
    return (
      ...
      <View style={{ marginTop: 20 }}>
        ...
        <Slider
          ...
          onValueChange={(amountSelected) => this.setState({ amountSelected }, () => {
            this.setState({ valueaR1: amountSelected })
          })}
          minimumValue={25000}
        ...
        <View style={{ flexDirection: "row", justifyContent: 'center', marginTop: 15 }}>
          ...
          <Text style={textEMI}>{`Amount Selected`}: {'\u20B9'}</Text>

          <TextInput
            style={{ height: 40, width: "20%", marginTop: -10, borderBottomWidth: 0.6, textAlign: 'center' }}
            keyboardType="numeric"
        ...
        <Text style={errorLabel}>{this.state.valueaR1 <= this.state.maxAmount == false ? `Please select the amount between 20,000 to ${this.state.maxAmount}.` : null}</Text>
        <View style={{ justifyContent: 'center', alignItems: 'center', marginVertical: 20 }}>
          <Button
            title={'Get Approved Loan \nAmount'}
            customContainerStyle={{ width: '55%' }}
            isDisabled={this.state.amountSelected == 0 || this.state.valueaR1 <= this.state.maxAmount == false}
            onPress={() => {
              Keyboard.dismiss();
                ...
                this.state.processingFees.isValid ?
                  ...
                  this.props.getApprovedLoanAmount({
                    data: {
                      ...
                      islifeInsurance: this.state.selectedSourceType === 'Yes' ? true : false,
                      ismainapplicant: true,
        ...
        {this.state.valueaR != '' ?
          <Text style={[textEMI, { fontSize: 16, marginTop: 15 }]}>{` ${`Approved Loan Amount: ${'\u20B9'} ${(Number(this.state.valueaR)).toString().replace('-', '')}`}`} </Text>
          : null}
        {this.state.valueaR != '' ?
       ...
       {console.log("mmmmmmm",(Number(this.state.valueaR) , Number(this.state.processingCharges)).toString().replace('-', '') , Number( 18 /100 * (Number(this.state.processingFees.value))), Number(this.state.nachCharges.value) ,Number(this.state.podCharges.value) ,Number(this.state.dealerCharges.value) ,Number(this.state.bureauCharges.value) ,Number(this.state.otherCharges.value))}
        ...
        <Slider
          ...
          onValueChange={(valuetR) => this.setState({ valuetR }, () => { this.setState({ valueTenure: valuetR }) })}
          minimumValue={6}
          maximumValue={36}
          minimumTrackTintColor="#16244f"
          maximumTrackTintColor="#4a4a4a"
        ...
        <View style={{ flexDirection: "row", justifyContent: 'center' }}>
          ...
          <TextInput style={{
            ...
            onChangeText={(valuetR) => {
              if (!isNaN(valuetR)) {
                if (valuetR > 0) {
                  this.setState({ valuetR: valuetR, valueTenure: valuetR }, () => this.emiamount())
                }
              }
            }}
  ...
  renderCharges() {
    ...
    return (
      <View>
            ...
            {!this.state.nachCharges.isValid &&
              <Text style={errorLabel}>
                {(this.state.nachCharges.value === "" ||
                  ...
                  : `Nach Charges must be greater than ${this.state.nachChargesMaster}`}</Text>}
          </View>
            ...
            {!this.state.preEMI.isValid &&
              <Text style={errorLabel}>
                {(this.state.preEMI.value === "" ||
                  ...
                  : LOANDETAILS_CONST.VALID_PREEMI}</Text>}
          </View>
        </View>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
          <View style={{ width: '45%', marginTop: 15 }}>
            ...
            {!this.state.podCharges.isValid &&
              <Text style={errorLabel}>
                {(this.state.podCharges.value === "" ||
                  ...
                  : LOANDETAILS_CONST.VALID_PDD}</Text>}
          </View>
            ...
            {!this.state.convenienceCharges.isValid &&
              <Text style={errorLabel}>
                {(this.state.convenienceCharges.value === "" ||
                  ...
                  : LOANDETAILS_CONST.VALID_CONVENIENCE}</Text>}
          </View>
        </View>
            ...
            {!this.state.dealerCharges.isValid &&
              <Text style={errorLabel}>
                {(this.state.dealerCharges.value === "" ||
                  ...
                  : LOANDETAILS_CONST.VALID_DEALERCHARGES}</Text>}
          </View>
        </View>

        <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
          <View style={{ width: '45%', marginTop: 13 }}>
            <Text style={[textStyleDrop, { fontSize: 15, fontWeight: '900' }]}>PF Amount Type*</Text>
            <DropDownPicker
              // disabled={this.state.isViewOnly}
              disabled={true}
            ...
            {!this.state.stampDuty.isValid &&
              <Text style={errorLabel}>
                {(this.state.stampDuty.value === "" ||
                  ...
                  : `Stamp Charges must be greater than ${this.state.stampDutyMaster}`}</Text>}
            ...
            {!this.state.bureauCharges.isValid &&
              <Text style={errorLabel}>
                {(this.state.bureauCharges.value === "" ||
                  ...
                  : LOANDETAILS_CONST.VALID_BUREAUCHARGES}</Text>}
          </View>
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
          <View style={{ width: '45%', marginTop: 13 }}>
            <Text style={[textStyleDrop, { fontSize: 15, fontWeight: '900' }]}>Payout Type</Text>
            <DropDownPicker
              // disabled={true}
              disabled={this.state.isViewOnly}
              items={this.state.dropDownDealerPayout}
            ...
            {!this.state.dealerPayout.isValid &&
              <Text style={errorLabel}>
                {(this.state.dealerPayout.value === "" ||
                  ...
                  : LOANDETAILS_CONST.VALID_DEALERPAYOUT}</Text>}
          </View>
        </View>
      </View>
    )
  ...
  renderOPDCoverage() {
    const {
      loanTextMain, radioContainer2, inputStyle, loanMainView, radioContainer, errorLabel, separatorStyle,
      addressStyle, labelDateOfIncStyle, inputTextPasswordStyle, calendarIconWithText, textForInputStyle

    } = LoanDetailsStyles;
    return (
      <View style={loanMainView}>
        <Text style={[loanTextMain, { color: colors.COLOR_LIGHT_NAVY_BLUE }]}>{'OPD Coverage'}</Text>
        ...
        {!this.state.selectedSourceValid3 && (
          <Text style={errorLabel}>{this.state.selectedSourceType3 === "" ? 'Please Select an Option' : null}</Text>
        )}
        ...
        <View style={{ marginTop: 15, }}>
          <Text style={loanTextMain}>OPD amount to be added to loan amount?</Text>
          <View style={radioContainer2}>{this.renderOPDAmountRadioButton()}</View>
        </View>
        <View style={[{ height: 1 }]} />
        {!this.state.selectedSourceValid4 && (
          <Text style={errorLabel}>{this.state.selectedSourceType4 === "" ? 'Please Select an Option' : null}</Text>
        )}
      </View>
    )
  }
    ...
    return (
      <View style={loanMainView}>
        ...
        {!this.state.selectedSourceValid && (
          <Text style={errorLabel}>{this.state.selectedSourceType === "" ? 'Please Select an Option' : null}</Text>
        )}
      </View>
    )
  }
  ...
  renderNomineeDetails() {
    const {
      loanTextMain, addressStyle, labelDateOfIncStyle, errorLabel, inputStyle, separatorStyle, textInputStyleNominee, radioContainer2, inputTextPasswordStyle, calendarIconWithText, textForInputStyle
    } = LoanDetailsStyles;
    return (
      <View>
        <View style={{ marginTop: 15 }}>
          <FloatingLabelInput
            editable={this.state.premiumAmountFreez ? false : !this.state.isViewOnly}
        ...
        <View style={{ marginTop: 15, }}>
          ...
          <Text style={loanTextMain}>Premium amount to be added to loan amount?</Text>
          <View style={radioContainer2}>{this.renderRadioButton()}</View>
        </View>
        <View style={[{ height: 1 }]} />
        {!this.state.selectedSourceValid2 && (
          <Text style={errorLabel}>{this.state.selectedSourceType2 === "" ? 'Please Select an Option' : null}</Text>
        )}
        {/* {this.state.selectedSourceType2 == "" &&
        ...
        {!this.state.dobValid && (
          <Text style={errorLabel}>{this.state.dateOfIncorporation === null ? LOANDETAILS_CONST.MANDATORY_DATEOFBIRTH : null}</Text>
        )}
