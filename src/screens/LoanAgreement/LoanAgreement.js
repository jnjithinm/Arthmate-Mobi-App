...
import { View, Text, ScrollView, Image, StatusBar, Platform, TouchableOpacity, Modal, Linking, PermissionsAndroid, Alert, AppState } from 'react-native';
...
import { ARROW_DOWN, ATTACHMENT, UP_ARROW, DOWN_ARROW } from '../../constants/imgConsts';
...
class LoanAgreement extends Component {
        ...
        this.props.getEserviceCommonData({
            ...
            callback: (response) => {
                ...
                this.setState({
                    ...
                    uploadLoanUploaded: response?.data?.loanAgreementFilePath ? true : false,
                ...
                if (response.data.type === 'esign') {
                    this.setState({
                        ...
                        esignFilePath: response?.data.filePath.replace('/var/www/html', uatURL.URL) || "",
                        filePathInResponse: response.data.loanAgreementFilePath == undefined ? "" : response.data.loanAgreementFilePath?.replace('/var/www/html', uatURL.URL),
                        uploadLoanName: response?.data?.loanAgreementFilePath == undefined ? { docName: null } : { docName: response?.data?.loanAgreementFilePath.split('\\').pop().split('/').pop() },
                        buttonDisable: response?.data?.loanAgreementFilePath ? true : false
                    })
                ...
                if (response?.data?.type === 'estamp') {
                    this.setState({
                        ...
                        disableProceed: true,
                        esignFilePath: response?.data?.filePath.replace('/var/www/html', uatURL.uatURL) || ""
                    })
                }
                this.loanSummary();
            }
        ...
        if (Platform.OS == 'ios') {
            ...
            try {
                ...
                if (granted === PermissionsAndroid.RESULTS.GRANTED) {
                    ...
                    handleError('You need to give storage permission to download the file');
                }
            } catch (err) {
                console.warn(err);
            }
    ...
    actualDownload = async () => {
        var link = this.state.downloadLoanAgreementPDF.replace('/var/www/html', uatURL.URL)
        var fileName = `Loan Agreement ${this.state.getCommonData.leadName + Date.now()}.pdf`
        if (link) {
            if (Platform.OS === 'android') {
                ...
                const downloadDest = `${RNFS.DocumentDirectoryPath}/${spiltStr[spiltStr.length - 1]}.pdf`;
                RNFetchBlob.config({
                    fileCache: true,
                    path: downloadDest,
                })
    ...
    actualDownload1 = async () => {
        ...
        var fileName = `Loan Agreement Esign ${this.state.getCommonData.leadName + Date.now()}.pdf`
        if (link) {
            if (Platform.OS === 'android') {
                const android = RNFetchBlob.android
                try {
                    ...
                    <View style={mainContainer}>
                        ...
                        <Modal
                            ...
                            <TouchableOpacity
                                ...
                                <Text style={modalText}>Click to close</Text></TouchableOpacity>
                        ...
                        <Modal
                            ...
                            <View style={modalView1}>
                                <Text style={text}>{"Do you Really want to proceed to Loan Agreement. Remember this action is irreversible this can't be undone."}</Text>
                                <View style={{ flexDirection: "row", justifyContent: "space-around", alignItems: "center" }}>
                                    ...
                                    <TouchableOpacity
                                        ...
                                        <Text style={[text, { color: colors.COLOR_LIGHT_NAVY_BLUE, textAlign: "center" }]}>No</Text>
                                    </TouchableOpacity>
                                    ...
                                    {value.title === "eSign Only" &&
                                        <View style={{ flex: 1, justifyContent: 'flex-start' }}>
                                            <DropDownPicker
                                                ...
                                                placeholder={LOAN_AGREEMENT_CONST.PLACEHOLDER_DROPDOWN_FAMILY}
                                                dropDownStyle={{ backgroundColor: '#ffffff' }}
                                                onChangeItem={item => this.setState({ selectedDropDownESign: { ...item, isValid: true } }, () => {
                                    ...
                                    {value.title === "eSign and eStamp Both" &&
                                        <View style={{ flex: 1, justifyContent: 'flex-start' }}>
                                            <DropDownPicker
                                                ...
                                                placeholder={LOAN_AGREEMENT_CONST.PLACEHOLDER_DROPDOWN_FAMILY}
                                                dropDownStyle={{ backgroundColor: '#ffffff' }}
                                                onChangeItem={item => this.setState({ selectedDropDownBoth: { ...item, isValid: true } }, () => {
                        ...
                        </View>
                        {
                            ...
                            <View>
                                <Text style={agreementLetter}>{LOAN_AGREEMENT_CONST.AGREEMENT_LETTER}</Text>
                                ...
                                >
                                    <Text style={modalText}>Click To Open PDF</Text></TouchableOpacity>
                            </View>
                        }
                        {this.state.esignFilePath != "" && this.state.esignFilePath != undefined &&
                            < View style={[mainSalaryView, { marginRight: 15 }]}>
                                <Image source={ATTACHMENT} style={plusImageStyle1} />
                                <Text style={salarySlipName}> {this.state.esignFilePath.split('\\').pop().split('/').pop()}
                                </Text>
                        ...
                        <View style={buttonContainerProceed}>
                            <Button title={LOAN_AGREEMENT_CONST.BUTTON_PROCEED}
                                ...
                                onPress={() => {
                                    ...
                                    if (!this.state.isViewOnly) {
                                        if (this.state.selectedSourceType === "eStamp Only") {
                                        ...
                                        else if (this.state.selectedSourceType === "eSign Only" && this.state.selectedDropDownESign.value != null) {
                                            this.props.requestStampPaper({
                                                data: {
                                                    applicantUniqueId: this.state.applicantUniqueId,
                                                    ismainapplicant: true,
                                        ...
                                        } else if (this.state.selectedSourceType === "eSign and eStamp Both" && this.state.selectedDropDownBoth.value != null) {
                                            this.props.requestStampPaper({
                                                data: {
                                                    applicantUniqueId: this.state.applicantUniqueId,
                                                    ismainapplicant: true,
                                            ...
                                            this.state.selectedSourceType == "" && handleError('Please select radio button.') ? null : handleError('Please select dropdown value')
                                        }
                                    } else {
                                        handleWarning("User access denied")
                                    }
                        ...
                        <Text style={agreementGreet}>{LOAN_AGREEMENT_CONST.LOAN_AGREEMENT}</Text>
                        <View style={buttonContainer}>
                            <Button style={touchableButtonStyle}
                                // isDisabled={false}
                                ...
                                <Icon style={{ marginTop: 15, marginLeft: 20, alignSelf: "center" }}
                                    size={25}
                                    name="pluscircle"
                                    type="antdesign"
                                    color={'white'}
                            ...
                            <View style={{ alignSelf: "center", marginLeft: 20 }}>
                                <Tooltip
                                    popover={<Text>Download agreement for manual Stamping/Signing.</Text>}
                                    backgroundColor={colors.COLOR_LILAC}
                                    skipAndroidStatusBar={true}
                                    overlayColor={"transparent"}
                                    withPointer={true}
                        ...
                        {this.state.downloadLoanAgreementPDF != "" && this.state.downloadLoanAgreementPDF != undefined &&
                            < View style={[mainSalaryView, { marginRight: 15 }]}>
                                <Image source={ATTACHMENT} style={plusImageStyle1} />
                                <Text style={salarySlipName}> {this.state.downloadLoanAgreementPDF.split('\\').pop().split('/').pop()}
                                </Text>
                        ...
                        <View style={buttonSecondContainer}>
                                    ...
                                    if (!this.state.isViewOnly) {
                                        ...
                                        const callback = (response) => {
                                            ...
                                            this.setState({
                                                ...
                                                filePathInResponse: response?.loanAgreementFilePath?.replace('/var/www/html', uatURL.URL) || "",
                                                buttonDisable: response?.data?.loanAgreementFilePath ? true : false,
                                                uploadLoanName: response?.data?.loanAgreementFilePath !== undefined ? { docName: response?.data?.loanAgreementFilePath.split('\\').pop().split('/').pop() } : { docName: null },
                            ...
                            <View style={{ flexDirection: 'row', alignSelf: "center", marginLeft: 20 }}>
                                <Tooltip
                                    popover={<Text>Upload agreement,duty signed by all the stakeholders.</Text>}
                                    backgroundColor={colors.COLOR_LILAC}
                                    skipAndroidStatusBar={true}
                                    overlayColor={"transparent"}
                                    withPointer={true}
                        ...
                        {this.state.uploadLoanName.docName !== null && this.state.uploadLoanName.docName !== undefined &&
                            < View style={mainSalaryView}>
                                <Image source={ATTACHMENT} style={plusImageStyle1} />
                                <TouchableOpacity style={{ width: '70%', marginTop: 10 }} onPress={() => {
                                    Linking.openURL(this.state.filePathInResponse)
                                }
                                }>
                                    <Text style={[salarySlipName, { alignSelf: 'center' }]}>{this.state.uploadLoanName?.docName}</Text>
                                </TouchableOpacity>
                                <Icon onPress={() => {
                                    if (!this.state.isViewOnly) {
