...
import { View, Text, TouchableOpacity, ScrollView, Image, StatusBar, Keyboard } from 'react-native';
...
import { BUSINESSDETAILS_CONST } from "../../constants/screenConst";
...
class BusinessDetails extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            selectedSectorItem: {
                value: null, sector_code: null,
                label: null, isValid: true,
            },
            selectedSegmentItem: {
                value: null,
                label: null, isValid: true,
    ...
    isIFSCCode() {
        ...
        if (this.state.ifscCode?.value != "" && this.state.ifscCode?.value != null && ifscCode.test(this.state.ifscCode?.value)) {
            valid = true;
        }
        this.setState({
            ifscCode: {
    ...
    isAccountNumber() {
        ...
        if (this.state.accountNumber?.value != "" && this.state.accountNumber?.value != null && turnoverRegex.test(this.state.accountNumber?.value)) {
            valid = true;
        }
        this.setState({
            accountNumber: {
    ...
    isNetWorth() {
        ...
        if (this.state.netWorth?.value != "" && this.state.netWorth?.value != null && networthRegex.test(this.state.netWorth?.value)) {
            valid = true;
        }
        this.setState({
            netWorth: {
                    ...
                    this.setState({
                        ...
                        () => {
                            if (
                                ...
                                var code = "";
                                this.state.dropDownItemSector.forEach(
                                    (item) => {
    ...
    renderDropDownSector() {
        const { viewDrop2, separatorStyle, textStyle, textStyle1, textStyleDrop, errorLabel } = BusinessDetailsStyles;
                                    ...
                                    hideBottomControls: true,
                                        ...
                                        ImageResizer.createResizedImage(image.path, 1200, 1200, 'JPEG', 100, 0)
                                            .then((response) => {
                                                ...
                                                zipFileUploadSelfie(response, dataToApi, this.props.uploadSelfie, callback)
                                            }).catch(err => {
                                            });
                    ...
                    onChangeItem={item => this.setState({
                        ...
                        this.props.businessSUBINDUSTRY({
                            ...
                            callback: (response) => {
                                ...
                                this.setState({ dropDownItemSubIndustry: [...tempSubIndustryArray] || [] })
                            }
                        });
                    })}
    ...
    renderDropDownIndustry() {
        const { viewDrop3, separatorStyle, textStyle, textStyle1, textStyleDrop, errorLabel } = BusinessDetailsStyles;
        return (
    ...
    renderTurnover() {
        const { viewDrop1, separatorStyle, inputStyle, inputTextStyle, errorLabel } = BusinessDetailsStyles;
        return (
            <View style={viewDrop1}>
                ...
                />
                <View style={separatorStyle} />
                {!this.state.turnOver.isValid && <Text style={errorLabel}>{(this.state.turnOver.value === "" || this.state.turnOver.value === null) ?
                    BUSINESSDETAILS_CONST.MANDATORY_TURNOVER : BUSINESSDETAILS_CONST.INVALID_TURNOVER}</Text>}
            </View>
        )
    }
    renderNetWorth() {
        const { separatorStyle, errorLabel, inputStyle, inputTextStyle } = BusinessDetailsStyles;
        return (
            <View style={{ marginTop: 10, marginBottom: 10 }}>
                ...
                />
                <View style={separatorStyle} />
                {!this.state.netWorth.isValid && <Text style={errorLabel}>
                    {BUSINESSDETAILS_CONST.INVALID_NETWORTH}</Text>}
            </View>
        )
    }

    renderDropDownSubIndustry() {
        const { viewDrop4, separatorStyle, textStyle, textStyle1, textStyleDrop, errorLabel } = BusinessDetailsStyles;
    ...
    renderDropDownSegment() {
        const { viewDrop, separatorStyle, textStyle, textStyle1, textStyleDrop, errorLabel } = BusinessDetailsStyles;
    ...
    renderBankStuff() {
        const { viewDrop, flexRowStyle,
            verifiedTextStyle,
            verifiedTickImageStyle, separatorStyle, textStyle, textStyle1, textStyleDrop, errorLabel, inputStyle, inputTextStyle } = BusinessDetailsStyles;
        ...
        return (
            <View style={viewDrop}>
                ...
                />
                <View style={separatorStyle} />
                {!this.state.selectedAccountType.isValid && <Text style={errorLabel}>Account type is mandatory.</Text>}
                ...
                <View style={{ marginTop: 30, marginBottom: 10 }}>
                    ...
                    />
                    <View style={separatorStyle} />
                    {!this.state.accountNumber.isValid && <Text style={errorLabel}>{(this.state.accountNumber.value === "" || this.state.accountNumber.value === null) ?
                        BUSINESSDETAILS_CONST.MANDATORY_ACCOUNTNUMBER : BUSINESSDETAILS_CONST.INVALID_NETWORTH}</Text>}
                </View>
                <View style={{ marginTop: 30, marginBottom: 10 }}>
                    <FloatingLabelInput
                    ...
                    />
                    <View style={separatorStyle} />
                    {!this.state.ifscCode.isValid && <Text style={errorLabel}>{(this.state.ifscCode.value === "" || this.state.ifscCode.value === null) ?
                        BUSINESSDETAILS_CONST.MANDATORY_IFSC : BUSINESSDETAILS_CONST.INVALID_NETWORTH}</Text>}
                </View>
                {this.state.isVerified ?
                    ...
                    <View style={{ width: '50%', alignSelf: 'center', marginTop: 10 }}>
                        {this.state.accountNumber.value != "" && this.state.ifscCode.value != "" && this.state.accountNumber.value != null && this.state.ifscCode.value != null ?
                            <Button
                                ...
                                onPress={() => {
                                    ...
                                    if (
                                        this.state.accountNumber.value != "" && this.state.accountNumber.value != null && this.state.accountNumber.isValid &&
                                        this.state.ifscCode.value != "" && this.state.ifscCode.value != null && this.state.ifscCode.isValid) {
                                        const dataToAPI = {
                                            applicantUniqueId: this.state.applicantUniqueId,
                ...
                <View style={{ marginTop: 30, marginBottom: 10 }}>
                    ...
                    />
                    <View style={separatorStyle} />
                    {!this.state.bankName.isValid && <Text style={errorLabel}>{(this.state.bankName.value === "" || this.state.bankName.value === null) ?
                        BUSINESSDETAILS_CONST.MANDATORY_BANKNAME : BUSINESSDETAILS_CONST.INVALID_NETWORTH}</Text>}
                </View>
                <View style={{ marginTop: 30, marginBottom: 10 }}>
                    <FloatingLabelInput
                    ...
                    />
                    <View style={separatorStyle} />
                    {!this.state.bankName.isValid && <Text style={errorLabel}>{
    ...
    render() {
        const {
            mainContainer, mainHeadingText, buttonContainer, cancelButtonStyle, cancelButtonTitleStyle
        ...
        return (
            <WaveBackground>
                ...
                <View style={{ alignContent: "center" }}>
                    <DottedProgressBar
                        totalSteps={this.state.iscoapplicant || this.state.isguarantor ? [1, 2, 3] : [1, 2, 3, 4, 5]}
                        currentIndex={3} />
                ...
                <ScrollView style={{ marginBottom: 20 }}>
                    <View style={mainContainer}>
                        {this.state.dropDownItemSector.length !== 0 ? this.renderDropDownSector() : null}
                            ...
                            <Button
                                ...
                                onPress={() => {
                                    ...
                                    if(this.state.filePath == null){
                                        ...
                                        this.state.selectedAccountType.value != null && this.state.selectedAccountType.value != "" &&
                                        this.state.accountNumber.value != "" && this.state.accountNumber.value != null && this.state.accountNumber.isValid &&
                                        this.state.ifscCode.value != "" && this.state.ifscCode.value != null && this.state.ifscCode.isValid &&
                                        this.state.bankName.value != "" && this.state.bankName.value != null && this.state.bankName.isValid) {
                                        const dataToAPI = {
                                            applicant_uniqueid: this.state.iscoapplicant || this.state.isguarantor ? this.state.coapplicantUniqueId : this.state.applicantUniqueId,
                                            ismainapplicant: this.state.ismainapplicant,
                                            isguarantor: this.state.isguarantor,
                                        ...
                                        if (this.state.netWorth.value != undefined && this.state.netWorth.value != null && this.state.netWorth.value != '') {
                                            dataToAPI['monthlyIncome'] = this.state.netWorth.value.replace(/,/g, "")
                                        }
                                        if (this.state.selectedSectorItem.value != null && this.state.selectedSectorItem.value != "" && this.state.selectedSectorItem.value != undefined) {
                                            dataToAPI['sector'] = this.state.selectedSectorItem.value
                                        }
                                        if (this.state.selectedSegmentItem.value != null && this.state.selectedSegmentItem.value != "" && this.state.selectedSegmentItem.value != undefined) {

                                            dataToAPI['segment'] = this.state.selectedSegmentItem.value
                                        }
                                        if (this.state.selectedIndustryItem.value != null && this.state.selectedIndustryItem.value != "" && this.state.selectedIndustryItem.value != undefined) {
                                            dataToAPI['industry'] = this.state.selectedIndustryItem.value
                                        }
                                        if (this.state.selectedSubIndustry.value != null && this.state.selectedSubIndustry.value != "" && this.state.selectedSubIndustry.value != undefined) {

                                            dataToAPI['subindustry'] = this.state.selectedSubIndustry.value
                                        }
